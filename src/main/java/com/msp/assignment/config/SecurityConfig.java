        ...
        try
        {
            http
                    .authorizeHttpRequests(authorize -> authorize
                                    .requestMatchers("/api/security/**").permitAll() // Allow access to /api/security/**
...
//                            .invalidSessionUrl("/api/security/user-login")); // Redirects to login if the session is invalid
            ...
            ;
            http.csrf().disable(); // Disable CSRF for simplicity, you might want to enable it in production
	        return http.build();
		}
        catch (Exception e)
        {
